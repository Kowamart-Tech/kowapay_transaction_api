...
const Airtime = {
    async createAirtimeTransaction(tx:IAirtime){
        const query = `
  ...
  return result.rows[0];
    ...
    async getUserAirtimeById(id:any){
       const result = await pool.query("SELECT * FROM airtime_transactions WHERE user_id =$1", [id]);
        return result.rows;
    }
